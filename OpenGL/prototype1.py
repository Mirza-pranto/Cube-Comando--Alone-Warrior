...
def draw_floor_with_boundaries():
    grid_size = 13
    tile_size = 80
    half_size = grid_size * tile_size / 2.0
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            x = i * tile_size - half_size
            y = j * tile_size - half_size
...
def will_bullet_hit(px, py, dir_x, dir_y, ex, ey, hit_radius=15):
    ...
    # Project enemy vector onto bullet direction
    dot = vx * dir_x + vy * dir_y

    if dot < 0:
        return False  # Enemy is behind

    # Closest point on the bullet path to the enemy
    closest_x = px + dot * dir_x
    closest_y = py + dot * dir_y

    # Distance from that closest point to the enemy
...
def draw_bullet(bullet):
    ...
    glPopMatrix()
def reset_game():
    global player_pos, bullets, enemy_positions, player_life, missed_bullets, game_over, score
    player_pos = [0, 0, 0]
    bullets = []
    enemy_positions = [spawn_enemy() for _ in range(5)]
    player_life = 5
...
def check_collisions():
    ...
    # Check kore bullet-enemy collision
    ...
    # Check kore enemy-player collisions
    px, py, pz = player_pos
    for i in range(len(enemy_positions)):
        ex, ey, ez = enemy_positions[i]
